...
from inverse_text_normalization.sa.data_loader_utils import get_abs_path
from inverse_text_normalization.sa.graph_utils import (
    NEMO_DIGIT,
    NEMO_SIGMA,
    NEMO_SPACE,
    GraphFst,
...
from inverse_text_normalization.sa.utils import num_to_word
...
class CardinalFst(GraphFst):
    ...
    def __init__(self):
        ...
        cents = pynini.accep("शत") |  pynini.accep("शतं") | pynini.accep("हंड्रेड") | pynini.accep("हन्ड्रड")
        thousands = pynini.accep("सहस्रम्") | pynini.accep("शशत") | pynini.accep("सहस्र") | pynini.accep("थाउजेंड") 
        lakhs = pynini.accep("लक्षः") | pynini.accep("लैक") | pynini.accep("लेक") | pynini.accep("लक्ष") | pynini.accep("लॅक्स") | pynini.accep("लॅख") | pynini.accep("लॅखस")
        crores = pynini.accep("कोटी") | pynini.accep("कोटिरूप्यकाणि") | pynini.accep("क्रोर")

        del_And = pynutil.delete(pynini.closure(pynini.accep("एंड"), 1 ,1 ))
        
        graph_hundred = pynini.cross("शत", "100") | pynini.cross("शतं", "100") | pynini.cross("हंड्रेड", "100") | pynini.cross("हन्ड्रड", "100")
        graph_thousand  = pynini.cross("सहस्रम्", "1000") | pynini.cross("शशत", "1000") | pynini.cross("सहस्र", "1000") | pynini.cross("थाउजेंड", "1000")
        graph_lakh = pynini.cross("लक्षः", "100000") | pynini.cross("लैक", "100000") | pynini.cross("लेक", "100000") | pynini.cross("लक्ष", "100000") | pynini.cross("लैक", "100000") | pynini.cross("लॅख", "100000") | pynini.cross("लॅखस", "100000")
        graph_crore = pynini.cross("कोटी", "10000000") | pynini.cross("कोटिरूप्यकाणि", "10000000") | pynini.cross("क्रोर", "10000000")
        ...
        graph_hundred_component_prefix_tens += pynini.union((graph_tens_en | graph_tens),
                                                            pynutil.insert("0") + delete_space + (graph_digit | pynutil.insert("0")))
        ...
        fst = pynini.union(fst, fst_crore, fst_lakh, graph_crore, graph_lakh, graph_thousand, graph_hundred, graph_zero, graph_multiples, graph_chars,graph_tens_en)

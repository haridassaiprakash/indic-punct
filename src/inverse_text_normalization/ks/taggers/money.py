...
class MoneyFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, decimal: GraphFst):
        ...
        cents_standalone = (
            ...
            + pynini.union(
                pynutil.add_weight(((NEMO_SIGMA - "एक") @ cardinal_graph), -0.7) @ add_leading_zero_to_double_digit
                + delete_space
                + pynutil.delete("पैसे"),
                pynini.cross("एक", "01") + delete_space + pynutil.delete("पैसा"),
            )
            + pynutil.insert("\"")
        )
